...
class ScoreRecord:
    __slots__ = ("time", "team", "operation", "buy_volume", "sell_volume", "etf_position", "etf_price",
                 "future_price", "total_fees", "balance", "profit_loss")

    def __init__(self, time: float, team: str, operation: str, buy_volume: int, sell_volume: int,
                 etf_position: int, etf_price: Optional[int], future_price: Optional[int], total_fees: int,
                 balance: int, profit_loss: int):
    ...
    def __iter__(self):
        return iter((round(self.time, 6),
                     ...
                     self.sell_volume,
                     self.etf_position,
                     self.etf_price//100 if self.etf_price is not None else None,
                     self.future_price//100 if self.future_price is not None else None,
                     round(self.total_fees/100.0, 2),
                     round(self.balance/100.0, 2),
                     round(self.profit_loss/100.0, 2)))
...
class ScoreBoardWriter:
    ...
    def breach(self, now: float, name: str, account: CompetitorAccount, etf_price: Optional[int],
        ...
        self.queue.put(
            ScoreRecord(now, name, "Breach", account.buy_volume, account.sell_volume, account.etf_position, etf_price,
                        future_price, account.total_fees, account.account_balance, account.profit_or_loss))
        ...
        if not self.finished:
            self.queue.put(
                ScoreRecord(now, name, "Disconnect", account.buy_volume, account.sell_volume, account.etf_position,
                            etf_price, future_price, account.total_fees, account.account_balance,
    ...
    def tick(self, now: float, name: str, account: CompetitorAccount, etf_price: Optional[int],
        ...
        self.queue.put(
            ScoreRecord(now, name, "Tick", account.buy_volume, account.sell_volume, account.etf_position, etf_price,
                        future_price, account.total_fees, account.account_balance, account.profit_or_loss))
    ...
    def writer(self, score_records_file: TextIO) -> None:
        ...
        try:
            with score_records_file:
                ...
                csv_writer.writerow(("Time,Team,Operation,BuyVolume,SellVolume,EtfPosition,EtfPrice,"
                                     "FuturePrice,TotalFees,AccountBalance,ProfitOrLoss").split(','))
