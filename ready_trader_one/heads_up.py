...
from .types import ICompetitor, IExecutionConnection


class HudConnection(Connection, IExecutionConnection):
    def __init__(self, loop: asyncio.AbstractEventLoop, match_events: MatchEvents,
                 competitor_manager: CompetitorManager, timer: Timer):
    ...
    def on_message(self, typ: int, data: bytes, start: int, length: int) -> None:
        ...
        now: float = self.__timer.advance()
...
class HeadsUpDisplayServer:
    def __init__(self, loop: asyncio.AbstractEventLoop, host: str, port: int, match_events: MatchEvents,
                 competitor_manager: CompetitorManager, timer: Timer):
    ...
    def __on_new_connection(self):
        ...
        return HudConnection(self.__event_loop, self.__match_events, self.__competitor_manager, self.__timer)
