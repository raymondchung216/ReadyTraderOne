...
from .types import IExecutionConnection
...
class ExecutionConnection(Connection, IExecutionConnection):
    ...
    def close(self):
        ...
        self.login_timeout.cancel()
        self.closing = True
        if not self._connection_transport.is_closing():
    ...
    def connection_lost(self, exc: Optional[Exception]) -> None:
        ...
        if self.competitor is not None:
            self.competitor.on_connection_lost(self.timer.advance())
...
class ExecutionServer:
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, host: str, port: int, competitor_manager: CompetitorManager,
                 limiter_factory: FrequencyLimiterFactory, timer: Timer):
    ...
    def __on_new_connection(self) -> ExecutionConnection:
        ...
        return ExecutionConnection(self.__event_loop, self.__competitor_manager, self.__limiter_factory.create(),
                                   self.__timer)

    async def start(self) -> None:
        """Start the server."""
        self.__logger.info("starting execution server: host=%s port=%d", self.host, self.port)
        self.__server = await self.__event_loop.create_server(self.__on_new_connection, self.host, self.port)
