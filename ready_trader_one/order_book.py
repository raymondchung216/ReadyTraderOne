...
class OrderBook(object):
    ...
    def __init__(self, instrument: Instrument, maker_fee: float, taker_fee: float):
        ...
        self.__levels: Dict[int, Deque[Order]] = {MINIMUM_BID: collections.deque(), MAXIMUM_ASK: collections.deque()}
        self.__total_volumes: Dict[int, int] = {MINIMUM_BID: 0, MAXIMUM_ASK: 0}
    ...
    def midpoint_price(self) -> int:
        ...
        bid_price: int = self.__bid_prices[-1]
        ask_price: int = self.__ask_prices[-1]
        return round((bid_price - ask_price) / 2.0) if bid_price > MINIMUM_BID and ask_price < MAXIMUM_ASK else None
    ...
    def trade_ask(self, now: float, order: Order) -> None:
        ...
        while order.remaining_volume > 0 and best_bid >= order.price and self.__total_volumes[best_bid] > 0:
            ...
            if self.__total_volumes[best_bid] == 0 and best_bid > MINIMUM_BID:
    ...
    def trade_bid(self, now: float, order: Order) -> None:
        ...
        while order.remaining_volume > 0 and best_ask <= order.price and self.__total_volumes[best_ask] > 0:
            ...
            if self.__total_volumes[best_ask] == 0 and best_ask < MAXIMUM_ASK:
    ...
    def trade_ticks(self, ask_prices: List[int], ask_volumes: List[int], bid_prices: List[int],
        ...
        if self.__ask_ticks or self.__bid_ticks:
            temp = sorted(self.__ask_ticks.keys())[:TOP_LEVEL_COUNT]
            temp += [0] * (TOP_LEVEL_COUNT - len(temp))
            ask_prices[:] = temp
            ask_volumes[:] = [self.__ask_ticks[p] for p in ask_prices]

            temp = sorted(self.__bid_ticks.keys(), reverse=True)[:TOP_LEVEL_COUNT]
            temp += [0] * (TOP_LEVEL_COUNT - len(temp))
            bid_prices[:] = temp
            bid_volumes[:] = [self.__bid_ticks[p] for p in bid_prices]
