...
class ActiveOrderTableModel(BaseTableModel):
    ...
    def __init__(self, team: str, parent: Optional[QtCore.QObject] = None):
        ...
        super().__init__(parent)
        self.team: str = team
        self.__orders: List[List[str, int, str, str, int, int]] = list()
...
class ProfitLossTableModel(BaseTableModel):
    ...
    _COLUMN_NAMES = ("Team", "Pos.", "Value", "TrdProfit", "TotFees", "NetProfit")
    ...
    _COLUMN_TOOLTIPS = ("Team name",
                        "ETF position (Future position will be the inverse of this number)",
    ...
    ETF_POSITION_COLUMN = _COLUMN_NAMES.index("Pos.")
    ...
    def data(self, index: QtCore.QModelIndex, role: int = Qt.DisplayRole) -> Any:
        ...
        if role == Qt.DisplayRole:
            ...
            if column == self.TEAM_NAME_COLUMN:
                return value
            elif column == self.ETF_POSITION_COLUMN:
                return str(value)
            else:
                return "%.2f" % value
        elif role == Qt.ForegroundRole:
    ...
    def on_login_occurred(self, team: str) -> None:
        ...
        if team not in self.__teams:
            ...
            self.__profits.append([team, 0, 0.0, 0.0, 0.0, 0.0])
            self._row_count += 1
            self.endInsertRows()
...
class TradeHistoryTableModel(BaseTableModel):
    ...
    _COLUMN_TOOLTIPS = ("Time when the trade occurred",
                        ...
                        "The fees paid (if positive) or collected (if negative) for this trade")

    def __init__(self, team: str, parent: Optional[QtCore.QObject] = None):
        """Initialise a new instance of the class."""
        super().__init__(parent)
        self.team: str = team
        self.__trades: List[Tuple[str, int, str, int, float, str]] = list()
    ...
    def on_trade_occurred(self, team: str, now: float, order_id: int, side: Side, volume: int, price: int,
        ...
        if team == self.team:
            self.beginInsertRows(QtCore.QModelIndex(), 0, 0)
            self._row_count += 1
            self.__trades.append(("%.3f" % now, order_id, ("Sell", "Buy")[side], volume, price // 100,
                                  "%.2f" % (-fee / 100.0)))
            self.endInsertRows()
