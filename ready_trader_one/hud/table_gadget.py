...
class ProfitLossTableGadget(BaseTableGadget):
    ...
    def set_model(self, table_model: ProfitLossTableModel) -> None:
        ...
        self._table_view.sortByColumn(self.__profit_model.NET_PROFIT_COLUMN)
        if self.__proxy_model.rowCount() > 0:
            self._table_view.setCurrentIndex(self.__proxy_model.createIndex(0, 0))
        else:
