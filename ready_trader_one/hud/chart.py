...
class BaseChartGadget(QtWidgets.QWidget):
    ...
    def _style_axes(self):
        ...
        chart.axisX().setLabelsColor(QtGui.QColor("#F0F0F0"))
        chart.axisY().setLabelFormat("%.0f")
        chart.axisY().setLabelsColor(QtGui.QColor("#F0F0F0"))
    ...
    def _update_y_axis(self, new_value: float) -> None:
        ...
        if new_value < self._smallest_y_value:
            self.chart.axisY().setMin(new_value - 1)
            self._smallest_y_value = new_value
        if new_value > self._largest_y_value:
            self.chart.axisY().setMax(new_value + 1)
...
class MidpointChartGadget(BaseChartGadget):
    ...
    def on_midpoint_price_changed(self, instrument: Instrument, time: float, mid_price: int) -> None:
