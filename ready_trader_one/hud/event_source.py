...
from ready_trader_one.messages import (AMEND_EVENT_MESSAGE, AMEND_EVENT_MESSAGE_SIZE, CANCEL_EVENT_MESSAGE,
...
from ready_trader_one.order_book import TOP_LEVEL_COUNT, Order, OrderBook
from ready_trader_one.types import Instrument, Lifespan, Side
...
class EventSource(QtCore.QObject):
    """A source of events for the Ready Trader One HUD to display."""
    ...
    midpoint_price_changed = QtCore.Signal(Instrument, float, int)  # instrument, time, price
    ...
    profit_loss_changed = QtCore.Signal(str, float, float, int, float, float)
...
class LiveEventSource(EventSource):
    ...
    def __init__(self, host: str, port: int, parent: Optional[QtCore.QObject] = None):
        """Initialise a new instance of the class."""
        super().__init__(parent)
    ...
    def on_login_event_message(self, name: str, competitor_id: int) -> None:
        ...
        self.__accounts[competitor_id] = CompetitorAccount(1.0, 0.002)
    ...
    def _on_timer_tick(self):
        ...
        for i in Instrument:
            midpoint_price: int = self.__order_books[i].midpoint_price()
        ...
        if future_price is not None and etf_price is not None:
            for competitor_id, account in self.__accounts.items():
                ...
                self.profit_loss_changed.emit(self.__teams[competitor_id], self.__now,
                                              ...
                                              account.account_balance / 100.0, account.total_fees / 100.0)
...
class RecordedEventSource(EventSource):
    ...
    @staticmethod
    def from_csv(file_object: TextIO, parent: Optional[QtCore.QObject] = None):
        """Create a new RecordedEventSource instance from a CSV file."""
        source = RecordedEventSource(parent)
        events = source.__events

        reader = csv.reader(file_object)
        next(reader)  # Skip header

        accounts: Dict[str, CompetitorAccount] = collections.defaultdict(lambda: CompetitorAccount(1.0, 0.002))
        ...
        for row in reader:
            ...
            if operation == "Insert":
                order = Order(order_id, Instrument(int(row[4])), Lifespan[row[8]], Side[row[5]],
                              int(float(row[7]) * 100.0), int(row[6]))
                ...
                side = Side[row[5]]
                volume = int(row[6])
                price = int(float(row[7]) * 100.0)
                fee = int(float(row[9]) * 100.0) if row[9] else 0
