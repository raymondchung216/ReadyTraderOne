...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF:
            ...
            if self.bid_id != 0 and self.bid_price != bid_prices[0]:
                self.send_cancel_order(self.bid_id)
                self.bid_id = 0

            if self.ask_id != 0 and self.ask_price != ask_prices[0]:
                self.send_cancel_order(self.ask_id)
                self.ask_id = 0
                ...
                if abs((ask_vwap - midprice)) > abs((bid_vwap - midprice)):
                    ...
                    if self.ask_id == 0:
                        if self.position + LOT_SIZE < (POSITION_LIMIT - LOT_SIZE):
                            self.send_insert_order(self.bid_id, Side.BUY, self.bid_price, LOT_SIZE*2, Lifespan.GOOD_FOR_DAY)
                            self.bids.add(self.bid_id)
                    else:
                        if self.position + LOT_SIZE < (POSITION_LIMIT - LOT_SIZE):
                            self.send_amend_order(self.ask_id, LOT_SIZE)
            ...
            if self.ask_id == 0 and new_ask_price != 0 and self.position > -POSITION_LIMIT:
                if (abs(bid_vwap - midprice)) > (abs(ask_vwap - midprice)):
                    ...
                    if self.ask_id == 0:
                        if self.position - LOT_SIZE > (-POSITION_LIMIT + LOT_SIZE):
                            self.send_insert_order(self.ask_id, Side.SELL, self.ask_price, LOT_SIZE*2, Lifespan.GOOD_FOR_DAY)
                            self.asks.add(self.ask_id)
                    else:
                        if self.position - LOT_SIZE > (-POSITION_LIMIT + LOT_SIZE):
                            self.send_amend_order(self.bid_id, LOT_SIZE)
...
#             sum_bid_volume = sum(bid_volumes)
#             sum_ask_volume = sum(ask_volumes)
#             quartile = 1 * 0.25
#             q_bid_vol = quartile * sum_bid_volume
#             q_ask_vol = quartile * sum_ask_volume

#             # get bid vwap
#             total_price = 0
#             total_volume = 0
#             for idx, vol in enumerate(bid_volumes):
#                 if total_volume + vol > q_bid_vol:
#                     total_price += (q_bid_vol - total_volume) * bid_prices[idx]
#                     break
#                 total_price += vol * bid_prices[idx]
#             bid_vwap = total_price / q_bid_vol

#             # get ask vwap
#             total_price = 0
#             total_volume = 0
#             for idx, vol in enumerate(ask_volumes):
#                 if total_volume + vol > q_ask_vol:
#                     total_price += (q_ask_vol - total_volume) * ask_prices[idx]
#                     break
#                 total_price += vol * ask_prices[idx]
#             ask_vwap = total_price / q_ask_vol

#             midprice = (bid_prices[0] + ask_prices[0]) / 2

#             # print(f"Q_bid: {q_bid_vol} bid_vwap: {bid_vwap}")
#             # print(f"Midprice = {midprice}")
#             # print(f"Q_ask: {q_ask_vol} ask_vwap: {ask_vwap}")
